...
static void add_test_materials(void) {
    ...
    m.unit_price = 68.0; m.total_stock = 3; m.min_stock = 5;
    material_add(&m);

    /* 严重不足的耗材 */
    strncpy(m.id, "M003", sizeof(m.id)-1);
    strncpy(m.name, "焊锡丝", sizeof(m.name)-1);
    m.category = CAT_CHEMICAL; m.attr = ATTR_DISPOSABLE;
    m.unit_price = 15.0; m.total_stock = 0; m.min_stock = 10;
    material_add(&m);
}
...
static void test_alert_print_output(void) {
