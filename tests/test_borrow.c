...
/** 构造一条 BorrowRecord */
static BorrowRecord make_borrow(const char *record_id, const char *student_id,
                                const char *material_id, int quantity) {
    BorrowRecord r;
    memset(&r, 0, sizeof(r));
...
static void test_create_borrow(void) {
    BorrowRecord r = make_borrow("BORROW-20260804-001", "2021001", "R001", 100);
...
static void test_disposable_reduces_stock(void) {
    ...
    /* 创建记录 */
    BorrowRecord r = make_borrow("BORROW-20260804-002", "2021002", "R001", 50);
    borrow_create(&r);
...
static void test_reusable_no_stock_change(void) {
    ...
    BorrowRecord r = make_borrow("BORROW-20260804-003", "2021003", "DEV001", 2);
    borrow_create(&r);
...
static void test_multiple_items_same_record(void) {
    ...
    BorrowRecord r1 = make_borrow("BORROW-20260804-004", "2021004", "R001", 10);
    borrow_create(&r1);

    BorrowRecord r2 = make_borrow("BORROW-20260804-004", "2021004", "DEV001", 1);
    borrow_create(&r2);
...
static void test_return_with_damage_and_scrap(void) {
    /* 先创建一条可循环工具的领用记录 */
    BorrowRecord r = make_borrow("BORROW-20260804-005", "2021005", "DEV001", 2);
...
static void test_return_normal_no_scrap(void) {
    /* 创建可循环工具领用 */
    BorrowRecord r = make_borrow("BORROW-20260804-006", "2021006", "DEV001", 1);
    borrow_create(&r);
...
static void test_overdue_detection(void) {
    /* 创建一条 8 天前的领用记录 */
    BorrowRecord r = make_borrow("BORROW-20260804-OVERDUE", "2021099", "DEV001", 1);
