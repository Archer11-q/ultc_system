...
/** 构造一个测试用耗材 */
static Material make_mat(const char* id, const char* name,
                          int category, int attr,
                          double price, int stock, int min_stock) {
    Material m;
    memset(&m, 0, sizeof(m));
...
static void test_add_material(void) {
    Material m = make_mat("M001", "电阻 10kΩ", CAT_ELECTRONIC,
                          ATTR_DISPOSABLE, 0.05, 500, 50);
...
static void test_add_duplicate_rejected(void) {
    Material m = make_mat("M001", "另一个电阻", CAT_ELECTRONIC,
                          ATTR_DISPOSABLE, 0.10, 100, 10);
    int ret = material_add(&m);
    assert(ret == -1);  /* 编号重复 */
    assert(material_count() == 1);
}
...
static void test_update_material(void) {
    Material m = make_mat("M001", "电阻 10kΩ 1/4W", CAT_ELECTRONIC,
                          ATTR_DISPOSABLE, 0.08, 800, 80);
...
static void test_update_nonexistent(void) {
    Material m = make_mat("NOEXIST", "x", CAT_TOOL, ATTR_REUSABLE,
                          1.0, 1, 1);
    int ret = material_update(&m);
    assert(ret == -1);
}
...
static void test_delete_material(void) {
    /* 先加一个临时耗材用于删除 */
    Material m = make_mat("TMP001", "临时耗材", CAT_CHEMICAL,
                          ATTR_DISPOSABLE, 1.0, 10, 2);
    material_add(&m);
