...
/* 测试数据文件列表 */
static const char *g_files[] = {"data/admin.dat",  "data/material.dat",  "data/scrap.dat",
...
/* 辅助：添加一个耗材 */
static void add_mat(const char *id, const char *name, int cat, int attr, double price, int stock,
                    int min_stock) {
    Material m;
    memset(&m, 0, sizeof(m));
...
static void borrow_item(const char *rid, const char *sid, const char *sname, const char *cls,
...
static void test_e2e_full_lifecycle(void) {
    ...
    /* 2. 新增耗材 */
    add_mat("R001", "电阻 10kΩ", CAT_ELECTRONIC, ATTR_DISPOSABLE, 0.05, 500, 50);
    add_mat("DEV001", "Arduino Uno", CAT_DEV_BOARD, ATTR_REUSABLE, 68.0, 20, 5);
    add_mat("WIRE01", "杜邦线 20cm", CAT_MECHANICAL, ATTR_DISPOSABLE, 1.5, 300, 50);
    ...
    tmp.total_stock = 30;
    material_update(&tmp);
    assert(material_find_by_id("R001")->total_stock < material_find_by_id("R001")->min_stock);
...
static void test_edge_all_alert(void) {
    ...
    /* 都应低于预警 */
    assert(material_find_by_id("R001")->total_stock < material_find_by_id("R001")->min_stock);
    assert(material_find_by_id("DEV001")->total_stock < material_find_by_id("DEV001")->min_stock);
    assert(material_find_by_id("WIRE01")->total_stock < material_find_by_id("WIRE01")->min_stock);
...
static void test_persistence_corrupted_file(void) {
    ...
    /* 恢复测试数据 */
    add_mat("RECOVER01", "恢复测试耗材", CAT_TOOL, ATTR_REUSABLE, 10.0, 100, 20);
...
static void test_concurrent_borrowing(void) {
    /* 添加一个共享耗材 */
    add_mat("SHARED", "共享耗材", CAT_CHEMICAL, ATTR_DISPOSABLE, 5.0, 1000, 100);
    ...
    for (int i = 0; i < count; i++) {
        if (strcmp(all[i].material_id, "SHARED") == 0)
            shared_count++;
    }
...
static void test_audit_trail_completeness(void) {
    ...
    /* 执行一系列操作 */
    /* 1. 新增耗材 */
    add_mat("AUDIT01", "审计测试耗材", CAT_TOOL, ATTR_REUSABLE, 10.0, 50, 10);
    /* 2. 领用 */
    BorrowRecord r;
    memset(&r, 0, sizeof(r));
...
int main(void) {
    ...
    printf("[E2E 全流程]\n");
    run_test("完整生命周期: 登录→新增→领用→归还→预警→盘点→统计", test_e2e_full_lifecycle);
