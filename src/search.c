...
void search_material_menu(void) {
    ...
    if (choice == 1) {
        ...
        printf("  柜号: %s\n", mat->cabinet);
        char date_buf[16];
        strftime(date_buf, sizeof(date_buf), "%Y-%m-%d",
                 localtime(&mat->purchase_date));
        printf("  采购日期: %s\n", date_buf);
        print_line();
    } else {
        /* 名称模糊搜索 */
        ...
        while (running) {
            ...
            for (int i = start; i < end; i++) {
                printf("  %-4d %-12s %-20s %-10s %-6s %6d %s\n",
                       i + 1, results[i].id, results[i].name,
                       material_category_name(results[i].category),
                       material_attr_name(results[i].attr),
                       results[i].total_stock, results[i].cabinet);
            }
            print_line();
            ...
            switch (buf[0]) {
            case 'n': case 'N': if (page < total_pages) page++; break;
            case 'p': case 'P': if (page > 1) page--; break;
...
void search_record_menu(void) {
    ...
    int count = 0;
    BorrowRecord* results = borrow_search(class_name, student_id,
                                           project_id, &count);
    if (!results || count == 0) {
        printf("\n  [提示] 未找到匹配的领用记录。\n");
        pause_screen();
    ...
    while (running) {
        ...
        printf("  筛选条件: ");
        if (class_name[0]) printf("班级=%s ", class_name);
        if (student_id[0]) printf("学号=%s ", student_id);
        if (project_id[0]) printf("项目=%s ", project_id);
        ...
        for (int i = start; i < end; i++) {
            ...
            printf("  %-4d %-22s %-10s %-10s %-12s %-6d %-8s %s\n",
                   ...
                   results[i].student_id, results[i].student_name,
                   results[i].material_id, results[i].quantity,
                   status_str, time_buf);
        }
        print_line();
        printf("  第 %d/%d 页", page, total_pages);
