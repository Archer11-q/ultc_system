...
static BorrowRecord *collect_filtered(borrow_predicate pred, const void *userdata, int *out_count) {
    *out_count = 0;
...
static int pred_by_student(const BorrowRecord *r, const void *userdata) {
    ...
    if (strcmp(r->student_id, f->student_id) != 0)
        return 0;
    if (f->exclude_returned && r->status == BORROW_RETURNED)
        return 0;
    return 1;
}
...
static int pred_search(const BorrowRecord *r, const void *userdata) {
    ...
    if (f->class_name[0] && strcmp(r->class_name, f->class_name) != 0)
        return 0;
    if (f->student_id[0] && strcmp(r->student_id, f->student_id) != 0)
        return 0;
    if (f->project_id[0] && strcmp(r->project_id, f->project_id) != 0)
        return 0;
    return 1;
}
...
int borrow_init(void) {
    int count = 0;
    BorrowRecord *arr = (BorrowRecord *)file_read_all(BORROW_FILE, sizeof(BorrowRecord), &count);
    if (count == -1) {
...
int borrow_create(const BorrowRecord *rec) {
    ...
    *node = *rec;
    ...
    append_borrow(node);
    save_borrows();
    audit_log(AUDIT_BORROW, rec->record_id, rec->student_name, rec->operator_name);
    return 0;
}
    ...
    while (p) {
        if (strcmp(p->record_id, record_id) == 0) {
            ...
            if (damage_note && damage_note[0] != '\0') {
                ...
                strncpy(p->damage_note, damage_note, sizeof(p->damage_note) - 1);
            } else {
...
int borrow_return_session(const char *record_id, const char *damage_note) {
    ...
    if (returned > 0) {
        ...
        audit_log(AUDIT_RETURN, record_id,
                  (damage_note && damage_note[0]) ? damage_note : "正常归还", NULL);
    }
    return returned;
}
...
BorrowRecord *borrow_search(const char *class_name, const char *student_id, const char *project_id,
                            int *out_count) {
    SearchFilter f = {class_name ? class_name : "", student_id ? student_id : "",
...
void borrow_list_page(int page, int *total_pages) {
    ...
    *total_pages = (total == 0) ? 1 : (total + PAGE_SIZE - 1) / PAGE_SIZE;
    ...
    while (p && shown < PAGE_SIZE) {
        ...
        printf("  %-22s %-10s %-10s %-12s %d %s\n", p->record_id, p->student_id, p->student_name,
               p->material_id, p->quantity, status_str);
        shown++;
        p = p->next;
    }
    print_line();
