...
static void csv_write_int(FILE* fp, int v) {
    fprintf(fp, "%d", v);
}

static void csv_write_double(FILE* fp, double v) {
    fprintf(fp, "%.2f", v);
}
...
static int parse_category(const char* s) {
    if (strcmp(s, "电子元器件") == 0 || strcmp(s, "0") == 0) return CAT_ELECTRONIC;
    if (strcmp(s, "电工工具")   == 0 || strcmp(s, "1") == 0) return CAT_TOOL;
    if (strcmp(s, "开发板")     == 0 || strcmp(s, "2") == 0) return CAT_DEV_BOARD;
    if (strcmp(s, "化学耗材")   == 0 || strcmp(s, "3") == 0) return CAT_CHEMICAL;
    if (strcmp(s, "机械零件")   == 0 || strcmp(s, "4") == 0) return CAT_MECHANICAL;
    return -1;
}

static int parse_attr(const char* s) {
    if (strcmp(s, "一次性")  == 0 || strcmp(s, "0") == 0) return ATTR_DISPOSABLE;
    if (strcmp(s, "可循环")  == 0 || strcmp(s, "1") == 0) return ATTR_REUSABLE;
    return -1;
}
...
int csv_import_materials(const char* filepath, const char* operator_name) {
    if (!filepath || !operator_name) return -1;
    ...
    if (bom_check != 0xEF) ungetc(bom_check, fp);
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (line_no == 1 && strstr(trimmed, "编号")
            && strstr(trimmed, "名称")) {
        ...
        mat.category=cat; mat.attr=attr;
        mat.unit_price=price; mat.total_stock=stock;
        mat.min_stock=min_stock;
        strncpy(mat.cabinet, cb, sizeof(mat.cabinet)-1);
        mat.purchase_date=mktime(&tm);

        if (material_add(&mat) == 0) imported++;
        else { errors++; }
    }
    fclose(fp);
...
int csv_export_materials(const char* filepath) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        strftime(date, sizeof(date), "%Y-%m-%d",
                 localtime(&mats[i].purchase_date));
...
int csv_export_purchase_list(const char* filepath) {
    ...
    int count = 0;
    Material* mats = material_get_all(&count);
    if (!mats || count == 0) { free(mats); return -1; }

    /* 筛选预警项 */
    int alert_count = 0;
    for (int i = 0; i < count; i++) {
        if (mats[i].total_stock < mats[i].min_stock) alert_count++;
    }
    if (alert_count == 0) {
        printf("\n  [提示] 无预警耗材，无需导出采购清单。\n");
        free(mats); return -1;
    ...
    for (int i = 0; i < count; i++) {
        if (mats[i].total_stock < mats[i].min_stock) {
            ...
            csv_write_field(fp, mats[i].id);        fputc(',', fp);
            csv_write_field(fp, mats[i].name);      fputc(',', fp);
            csv_write_field(fp, material_category_name(mats[i].category));
            fputc(',', fp);
            csv_write_int(fp, mats[i].total_stock); fputc(',', fp);
            csv_write_int(fp, mats[i].min_stock);   fputc(',', fp);
            csv_write_int(fp, suggest);             fputc(',', fp);
            csv_write_double(fp, mats[i].unit_price); fputc(',', fp);
            csv_write_double(fp, cost);             fputc('\n', fp);
        }
    }
    /* 汇总行 */
    fprintf(fp, ",,,,,,采购总金额:,");
...
int csv_export_borrow_records(const char* filepath) {
    ...
    FILE* fp = fopen(filepath, "w");
    if (!fp) { free(recs); return -1; }
    write_bom(fp);

    fprintf(fp, "领用单号,学号,姓名,班级,项目编号,耗材编号,数量,状态,领用时间,归还时间\n");
    for (int i = 0; i < count; i++) {
        csv_write_field(fp, recs[i].record_id);    fputc(',', fp);
        csv_write_field(fp, recs[i].student_id);   fputc(',', fp);
        csv_write_field(fp, recs[i].student_name); fputc(',', fp);
        csv_write_field(fp, recs[i].class_name);   fputc(',', fp);
        csv_write_field(fp, recs[i].project_id);   fputc(',', fp);
        csv_write_field(fp, recs[i].material_id);  fputc(',', fp);
        csv_write_int(fp, recs[i].quantity);       fputc(',', fp);

        const char* st;
        switch (recs[i].status) {
        case BORROW_ACTIVE: st="领用中"; break;
        case BORROW_RETURNED: st="已归还"; break;
        case BORROW_OVERDUE: st="逾期"; break;
        case BORROW_SCRAPPED: st="已报废"; break;
        default: st="未知";
        }
        csv_write_field(fp, st); fputc(',', fp);

        char tbuf[20];
        strftime(tbuf, sizeof(tbuf), "%Y-%m-%d %H:%M",
                 localtime(&recs[i].borrow_time));
        csv_write_field(fp, tbuf); fputc(',', fp);

        if (recs[i].return_time > 0) {
            strftime(tbuf, sizeof(tbuf), "%Y-%m-%d %H:%M",
                     localtime(&recs[i].return_time));
