...
static void append_audit(AuditRecord* node) {
    node->next = NULL;
    if (!g_audit_list) { g_audit_list = node; return; }
...
static int save_audits(void) {
    ...
    if (count == 0) { remove(AUDIT_FILE); return 0; }
...
int audit_init(void) {
    int count = 0;
    AuditRecord* arr = (AuditRecord*)file_read_all(AUDIT_FILE,
                                                    sizeof(AuditRecord),
                                                    &count);
    if (count == -1) {
...
void audit_shutdown(void) {
    save_audits();
    AuditRecord* p = g_audit_list;
    while (p) { AuditRecord* n = p->next; free(p); p = n; }
    g_audit_list = NULL;
}
...
void audit_log(int action, const char* target_id,
    ...
    if (!detail)    detail    = "";
    if (!operator_name) operator_name = auth_current_user();
    if (!operator_name || operator_name[0] == '\0') operator_name = "系统";
    ...
    strncat(rec->log_id, seq_str,
            sizeof(rec->log_id) - strlen(rec->log_id) - 1);

    rec->timestamp = now;
    strncpy(rec->operator_name, operator_name,
...
void audit_list_page(int page, int* total_pages,
    ...
    while (p) {
        int match = 1;
        if (filter_action >= 0 && p->action != filter_action) match = 0;
        if (filter_operator && filter_operator[0] != '\0'
            && strcmp(p->operator_name, filter_operator) != 0) match = 0;
        if (match) total++;
        p = p->next;
    }
...
const char* audit_action_name(int action) {
    switch (action) {
    case AUDIT_LOGIN:        return "登录";
    case AUDIT_LOGOUT:       return "登出";
    case AUDIT_MAT_ADD:      return "新增耗材";
    case AUDIT_MAT_EDIT:     return "修改耗材";
    case AUDIT_MAT_DELETE:   return "删除耗材";
    case AUDIT_MAT_SCRAP:    return "报废耗材";
    case AUDIT_BORROW:       return "学生领用";
    case AUDIT_RETURN:       return "耗材归还";
    case AUDIT_STOCKTAKE:    return "盘点修正";
    case AUDIT_ADMIN_ADD:    return "新增管理员";
    case AUDIT_ADMIN_DEL:    return "删除管理员";
    case AUDIT_ADMIN_CHPWD:  return "修改密码";
    case AUDIT_IMPORT:       return "CSV批量导入";
    default:                 return "未知";
    }
}
